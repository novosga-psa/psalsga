...
class AtendimentoController extends ModuleController
{
    ...
    public function index(Context $context)
    {
        ...
        $teste = $this->intervaloExiste($context);
        if( $teste->data[0]["emIntervalo"] === "N" ){
          $this->app()->view()->set('atendimento', $this->atendimentoService->atendimentoAndamento($usuario->getId()));
        }
        else {
          $this->app()->view()->set('emIntervalo', 5 );
        }
    ...
    /**
     * Gravar inicio intervalo do usuario
     * 
     * @param Novosga\Context $context
     */
    public function intervaloIniciar(Context $context)
    {
      $response = new JsonResponse();
      ...
      // verifica se ja esta atendendo alguem
      $emAtendimento = $this->atendimentoService->atendimentoAndamento($usuario->getId());
      if(! $emAtendimento) {
        // Abrir conexao com o banco de dados
        $connDB = new \PDO( \Novosga\Util\PDOConexao::strConexao() );

        // verificar se a tabela de intervalos existe, caso negativo criar tabela
        $this->intervaloTabela($connDB);

        $response = $this->intervaloExiste($context);
        
        // Verifica se já existe intervalo iniciado, caso negativo inicia intervalo 
        if( $response->data[0]["emIntervalo"] === 'N' ){
          $this->intervaloGravar($connDB, $usuario->getId());
          $response->success = true;
          $response->message = 'Intervalo liberado';
          $response->data = [
              [ 'emIntervalo' => 'S', 
                'usuario_id' => $usuario->getId(), 
                'inicio' => date("d/m/Y H:i") . 'h', 
                'final' => '' 
              ]
          ];
        }
        ...
        throw new Exception(_('Complete o atendimento antes de iniciar o intervalo'));
      }
    ...
    /**
     * Gravar encerramento intervalo do usuario
     * 
     * @param Novosga\Context $context
     */
    public function intervaloEncerrar(Context $context)
    {
      $response = new JsonResponse();
      ...
      $response->success = true;
      $response->message = 'Intervalo ENCERRADO';
      $response->data = [
              [ 'emIntervalo' => 'N',
                'usuario_id' => '', 
                'inicio' => '', 
    ...
    /**
     * Verificar a existencia de intervalo
     * 
     * @param Novosga\Context $context
     */
    public function intervaloExiste(Context $context)
    {
      $response = new JsonResponse();
      ...
      $dados = $this->intervaloDados( $connDB, $usuario->getId());
      if( ! $dados ){
          $response->success = true;
          $response->message = 'Intervalo NÃO Iniciado';
          $response->data = [
              [ 'emIntervalo' => 'N',
                'usuario_id' => '', 
                'inicio' => '', 
          ...
          $response->success = true;
          $response->message = 'Intervalo JÁ Iniciado';
          $response->data = [
              [ 'emIntervalo' => 'S',
    ...
    private function intervaloGravar( $conexao, $usuario_id )
    {
      $sql = "insert into psa_intervalo( "
             . "  usuario_id, inicio" 
             . " ) values (" 
    ...
    private function intervaloDados( $conexao, $usuario_id)
    {
      $sql = "select usuario_id "
           . ", TO_CHAR(inicio,'DD/MM/YYYY HH24:MI') || 'h' inicio"
           . ", TO_CHAR(final,'DD/MM/YYYY HH24:MI') || 'h' final from psa_intervalo "
           . " where usuario_id = :usuario_id "
    ...
    private function intervaloTabela( $conexao)
    {
      // verificar se a tabela de intervalos existe, caso negativo criar tabela
      $sql = "SELECT count(*) contador";
      $sql .= " FROM   information_schema.tables ";
      $sql .= " WHERE  upper(table_catalog) = 'NOVOSGA' ";
      $sql .= "  AND   upper(table_name) = 'PSA_INTERVALO'";
      ...
      $rs = $stm->fetchObject();
      if( $rs->contador == 0 ){
          $crt = "CREATE TABLE IF NOT EXISTS psa_intervalo ("
