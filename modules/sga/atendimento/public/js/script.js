...
SGA.Atendimento = {
    ...
    updateControls: function(status, atendimento) {
        ...
        switch (status) {
            ...
            case 4: // atendimento encerrado (faltando codificar)
                ...
                break;
            case 5: // Encerramento de Intervalo 
                $('#intervaloEncerrar').show();
                break;
        }
    },
    ...
    intervaloIniciar: function(btn){
        SGA.Atendimento.control({
            button: btn,
            enableDelay: 1000,
            action: 'intervaloIniciar', 
            success: function(response) {
              SGA.Atendimento.updateControls(5, response.data[0] );
            }
        });
    },
    
    intervaloEncerrar: function(btn){
        SGA.Atendimento.control({
            button: btn,
            enableDelay: 1000,
            action: 'intervaloEncerrar', 
            success: function(response) {
              SGA.Atendimento.init( 1 );
            }
        });
    }
            
    /*
    ,intervaloExiste: function(btn){
        SGA.Atendimento.control({
            cache: false,
            button: btn,
            enableDelay: 3000,
            action: 'intervaloExiste', 
            success: function(response) {
              if( response.success && response.data[0].emIntervalo === 'S' ){
                SGA.Atendimento.updateControls(5, response.data[0] );
              }
            }
        });
